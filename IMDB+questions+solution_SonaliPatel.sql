...
WITH	movies_with_one_genre	AS (SELECT movie_id
FROM	genre
GROUP BY	movie_id
...
-- movies_with_one_genre: 3289 1 rows in set
...
-- Action genre has the highest duration of 112.88 followed by romance and crime genres and lowest is horror geners.
...
-- movie_count: 1484 and genre_rank: 3
...
-- MIN_AVG_RATING: 1.0 MAX_AVG_RATING: 10.0 MIN_TOTAL_VOTES: 100
-- MAX_TOTAL_VOTES: 725138 MIN_MEDIAN_RATING: 1 MAX_MEDIAN_RATING: 10
...
-- avg_rating: 9.4 movie_rank: 10 10 rows in set
...
-- median_rating: 1 movie_count: 94  10 rows in set
-- median rating 7 have highest movies count 2257
...
-- production_company: National Theatre Live MOVIE_COUNT: 3  PROD_COMPANY_RANK: 1
...
-- All the movies belong to the top 3 genres are drama
-- Movies belongs to 3 Geners i.e Crime Drama, Action
...
-- Answer is Yes if German votes > Italian vote
...
-- name_nulls: 0 height_nulls: 17335 date_of_birth_nulls: 13431 known_for_movies_nulls: 15226
...
-- James Mangold , Joe Russo and Anthony Russo are top three directors in the top three genres
...
-- actor_name: Mammootty,  movie_count: 8	
-- actor_name: Mohanlal , movie_count: 5
...
-- Der müde Tod is the movie with rating 7.7 and average rating category is Hit movies
...
ON g.movie_id = m.id
WHERE category = 'ACTRESS'
AND avg_rating>8 AND genre = "Drama"
GROUP BY NAME )
...
(SELECT d.name_id,NAME,d.movie_id, duration, r.avg_rating,
